...
def recommend(media_format: str, media_type: str) -> None:
    ...
    df["genres"] = df["genres"].apply(
        lambda x: sorted([g.replace(" ", "") for g in x]) if x else []
    )
    df["tags"] = df["tags"].apply(
        lambda x: sorted([t.replace(" ", "") for t in x]) if x else []
    )
    df["synopsis"] = df["synopsis"].fillna("")
    df["synopsis"] = df["synopsis"].str.lower()

    df["synopsis"] = df["synopsis"].apply(
        lambda x: f"Synopsis: {x}".replace("\n", "") if x else ""
    )
    ...
    for media_id in df["_id"]:
        ...
        for row in recdf.itertuples():
            match = process.extractOne(
                row.title, unique, scorer=fuzz.partial_ratio, score_cutoff=90
            )
            if match is None:
        ...
        save(
            media_id, results.apply(lambda x: x.to_dict(), axis=1).tolist(), media_type
        )
